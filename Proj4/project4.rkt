...
(define(extField! fields)
		(if(null? fields) fields
		(let* ([loc1 (new-loc!)]
		[env (extField!(rest fields))])
		(extend-dict (first fields) loc1 env))))
...
(define (interp exp env cenv sto)
   (cond
      ...
      [(LambdaExp? exp)
           (values (ClosureV
					(LambdaExp-formal exp)
					(LambdaExp-body exp) 
					 env cenv)sto)] ;;; CHANGE #1 ** DONE **
      [(ApplyExp? exp)
          (let*-values 
            ...
            (if (ClosureV? v1)
               (let ([loc (new-loc!)])
                   (interp
					(ClosureV-body v1)
					(extend-dict (ClosureV-formal v1) loc (ClosureV-env v1)) 
					(ClosureV-cenv v1) (extend-dict loc v2 sto2))) ;;; CHANGE #2 ** DONE **              
      ...
      [(SeqExp? exp)
		(let-values
			([(v0 sto0) (interp (SeqExp-exp1 exp) env cenv sto)])
           (interp (SeqExp-exp2 exp) env cenv sto0))] ;;; CHANGE #3 ** DONE **
      [(CondExp? exp)
          (let-values 
      ...
      [(LetClassExp? exp)
          (interp
             ...
             env
             (extend-dict (LetClassExp-id exp) (LetClassExp-fields exp) cenv) 
			 ;;; CHANGE #4 ** DONE **
             sto)]
      [(NewExp? exp) 
          (let* ([id (NewExp-id exp)]
                 [fields (lookup-dict
                            ...
                            id cenv)]
                 [env0 (extField! fields)]  ;;; CHANGE #5 ** DONE: CHECK ME **
             )
          ...
          (let-values 
             ...
             (if (ObjectV? v0)
                (let* ([field (ReadExp-field exp)]
                      ...
                      (values (lookup-dict "ERROR" loc sto0 ) ;;; CHANGE #6 ** WIP **
                               sto0))  
      ...
      [(WriteExp? exp)
          (let*-values 
             ...
             (if (ObjectV? v0)
                (let* (
				[field (WriteExp-field exp)]
				[loc (lookup-dict 
                               (string-append "undeclared field " field) 
                               field (ObjectV-env v0))]
				)
                   (values v2(extend-dict loc v2 sto0)  ;;; CHANGE #7
                    ))
