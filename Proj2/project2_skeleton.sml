...
exception ReadingError of string 
(* CHANGE #4 *)
...
fun calculate_displacement id indices bounds = case (indices, bounds) of
 ...
 | ((index1 :: indices'), (bound1 :: bounds')) =>
           index1 (* CHANGE #9 *)
 |	((index1 :: indices'),(index2 :: indices'), (bound1 :: bounds'),(bound2 :: bounds')) =>		   
		id + index1 * bound1 + index2
...
|   CommExec (SeqC(cmd1,cmd2)) env state = (* CHANGE #2 *) (* DONE *)
      let val state1 = CommExec cmd1 env state
       ...
       in state2 end
|   CommExec (IfC(exp,cmd1,cmd2)) env state = (* CHANGE #7 *) (* DONE *)
          if ExpEval exp sto > 0
			then(CommExec cmd1 env state)
				else (CommExec cmd2 env state)
|   CommExec (WhileC(exp,cmd)) env state = (* CHANGE #8: *) (* DONE *)
        while (ExpEval exp sto) > 0 
			do (CommExec cmd env state)
...
|   CommExec (InputC lhs) env (sto,inp,outp) = (* CHANGE #6 *) (* DONE *)
       ((StoUpdate (LeftEval lhs env sto) (hd inp) sto),(tl inp), outp)
...
fun Interpret prog inp = ProgRun (parse prog) inp
   handle (* CHANGE #5 *)
	...
	| (ReadingError x) =>
		(print("*** error: "^x^" in an invalid syntax, cannot be read\n"); [0])
